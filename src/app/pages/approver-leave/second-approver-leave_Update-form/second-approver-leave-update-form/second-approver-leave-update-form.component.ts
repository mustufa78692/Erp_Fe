...
@Component({
  selector: 'app-second-approver-leave-update-form',
  templateUrl: './second-approver-leave-update-form.component.html',
  styleUrls: ['./second-approver-leave-update-form.component.scss']
})
export class SecondApproverLeaveUpdateFormComponent implements OnInit {
  ...
  // data:any
  public id :any
  constructor( private sanitizer: DomSanitizer,private formBuilder:FormBuilder, private services:ServicesService, private route:ActivatedRoute) { 
  ...
  ngOnInit(): void {
    ...
    this.services.getLeaveFormDetailsByleaveForm_id(this.id).subscribe((res) => {
     ...
     console.log(this.obj?.employeeId)
  ...
  public fetchIdThroughToken(){
    ...
    if (token) {
      ...
      this.services.personalInfo(userId).subscribe((success)=>{
        ...
        console.log(this.ownData?.firstName + this.ownData?.middleName + this.ownData?.lastName)
        // this.data=success;
        // // console.log(this.data.firstName)
        // this.data=success
...
public obj:any
  ...
  public initLeaveForm() {
    ...
    this.LeaveForm = this.formBuilder.group({
      ...
      approvalStatus: [this.obj?.approvalStatus],
      approvalRemarks: [this.obj?.approvalRemarks],
      managerEmail:[this.obj?.approvingManagerName],
      //
      hrName:[this.ownData?.firstName + this.ownData?.middleName + this.ownData?.lastName],
      hrApprovalStatus:[''] ,
      hrApprovalRemarks:[''],
      hrEmail: [this.ownData?.email],
...
if(this.LeaveForm.valid){
  ...
  this.services.updateLeaveFormByHr(this.id,formFileData).subscribe((successResponse)=>{
